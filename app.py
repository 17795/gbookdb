...
def search_book():
    ...
    SelectFrom='''SELECT book.ISBN,book.BookName,group_concat(DISTINCT author.Writer) AS Writer,
    ...
    if search_case == 'book_name':
        ...
        sql_query = SelectFrom+'''WHERE book.ISBN = author.ISBN AND book.ISBN =(
            SELECT ISBN FROM author WHERE (author.Writer) LIKE '%"+query_txt+"%') GROUP BY book.ISBN;'''
        data = select(sql_query)
