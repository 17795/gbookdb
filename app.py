...
def insert(sql_query):
    cur = con.cursor()
    cur.execute(sql_query)
    logger.info('***修改/新增/删除数据*** %s' % sql_query)
    con.commit()
    ...
    if argv == "default":
        ...
        if request.method == 'POST':
            ...
            if len(chkbox_values) != 0:
                ...
                print(OrderID)
                today = datetime.date.today()
                sql_query = "INSERT IGNORE INTO `order` VALUES ('"+str(OrderID)+"', '"+str(CustomerID)+"', '"+str(today)+"','not done', NULL, NULL)"
                insert(sql_query)
...
def show_order(argv):
    ...
    if argv == 'processEntry':
        ...
        if (Status == "done" or Status == "under stock" or Status == "in cart"):
            return render_template('manage_order.html', var = 'query+all')
        ...
        for i in range(len(Quantity)):
            if (Stock[i] is None or int(Stock[i]) < Quantity[i]):
                print (Entries[i], "is under stock")
        ...
        for i in range(len(Quantity)):
            ...
            insert(sql_query)
        CustomerID = select("SELECT CustomerID FROM `order` WHERE OrderID = '" + OrderID + "'")[0][0]
        Total = sum(Quantity)
        sql_query = "UPDATE customer SET RedemptionPoints = RedemptionPoints+" + str(Total) + " WHERE CustomerID = '" + str(CustomerID) + "';"
        insert(sql_query)
        return render_template('manage_order.html', var = 'query+all')
    if argv == 'returnEntry':
        print('return entry!')
...
def show_customer(argv):
    if argv[0:5] == 'query':
        ...
        rows = []
        for record in detail:
            rows.append({'CustomerID': record[0], 'CustomerName': record[1], 'Password': record[2], 'RedemptionPoints': record[3]})
        data = {'rows': rows}
        j_reslist = json.dumps(data)
        # print(j_reslist)
        return j_reslist
...
def show_sale_charts():
    ...
    # 用户订单数排行榜
    sql_query = "SELECT CustomerName,COUNT(*) FROM `customer` INNER JOIN `order` ON (`order`.CustomerID=customer.CustomerID) WHERE order.OrderID != 'in cart' GROUP BY `customer`.`CustomerID` ORDER BY COUNT(*) DESC"
    detail = select(sql_query)
    # 订单top10
    detail_top = detail[:10]
    active_name = ''
    ...
    for i in type_count.items():
        if i[0] is None:
            type_str = type_str + "'" + "未分类" + "',"
        else:
            type_str = type_str + "'" + i[0] + "',"
        if i[0] is None:
            type_count_str = type_count_str + "{value:" + str(i[1]) + ", name:'" + "未分类" + "'},\n"
        else:
