...
def Customer(argv):
    ...
    if argv == "default":
        ...
        for record in data:
            html_str += '<table> <div class="mytable">'
            html_str += f'<tr><th>订单号:</th><td><a href="/historyorder/{record[0]}" target="_parent"><span>{record[0]}</span></a></td></tr>'
        ...
        html_str += f'<input type="text" name="newpw1"></br>'
        html_str += f'<input type="text" name="newpw2"></br>'
        html_str += f'<input type="submit" value="修改密码"></br>'
        html_str += f'</form>'
        if request.method == 'POST':
        ...
        for record in data:
            html_str += '<table> <div class="mytable">'
            html_str += f'<tr><th>订单号:</th><td><a href="/messageorder/{record[0]}" target="_parent"><span>{record[0]}</span></a></td></tr>'
            html_str += f'<tr><th>日期:</th><td>{record[2]}</td></tr>'
            html_str += f'<tr><th>留言:</th><td>{record[4]}</td></tr>'
            html_str += f'<tr><th>回复:</th><td>{record[5]}</td></tr>'
            html_str += f'<tr><th>状态:</th><td>{record[3]}</td></tr>'
